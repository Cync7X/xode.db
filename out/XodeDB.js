...
const XodeDBConfig_1 = require("./lib/XodeDBConfig");
class XodeDB {
    /**
     * JSONDB Constructor
    ...
    constructor(filename, saveOnPush = true, humanReadable = false, separator = "/") {
        this.loaded = false;
        this.data = {};
        if (filename instanceof XodeDBConfig_1.Config) {
            this.config = filename;
        }
        else {
            this.config = new XodeDBConfig_1.Config(filename, saveOnPush, humanReadable, separator);
        }
     ...
     */
    count(dataPath) {
        const result = this.get(dataPath);
    ...
    filter(rootPath, callback) {
        const result = this.get(rootPath);
        if (Array.isArray(result)) {
...
exports.XodeDB = XodeDB;
//# sourceMappingURL=XodeDB.js.map